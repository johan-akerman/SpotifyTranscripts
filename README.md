...
More specifically, my REST API downloads a podcast in mp3 format and converts it into wov format. The wov file is divided into sentences (multiple smaller audio files) by listening for a silence that indicates the end of a sentence  (500 ms of audio below 14 decibels). Each new smaller audio file is sent into the Google Speech Recognition API that converts speech to text. As a result, my API returns each transcribed sentence together with it's start- and endtime. 
